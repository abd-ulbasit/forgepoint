...
this chart mirrors fp-auth's helpers verbatim except the chart name token.
...
{{/*
fp-pipeline-orchestrator.name — the chart's short name, overridable, truncated to
the 63-char DNS label limit. trimSuffix "-" avoids a trailing dash if truncation
lands on one.
*/}}
{{- define "fp-pipeline-orchestrator.name" -}}
{{- default .Chart.Name .Values.nameOverride | trunc 63 | trimSuffix "-" -}}
{{- end -}}

{{/*
fp-pipeline-orchestrator.fullname — the fully-qualified resource name (release + chart).
WHY this exact dance (the canonical Helm idiom):
  ...
  - else if the release name already contains the chart name, don't double it
    (release "fp-pipeline-orchestrator" + chart "fp-pipeline-orchestrator" →
    "fp-pipeline-orchestrator", not "...-...").
  - else join release + chart.
Always trunc 63 / trimSuffix "-" for DNS-1123 compliance.
*/}}
{{- define "fp-pipeline-orchestrator.fullname" -}}
...
{{/*
fp-pipeline-orchestrator.chart — "name-version" sanitized for the helm.sh/chart label
(+ → _ because '+' is illegal in a label value; appears in SemVer build metadata).
*/}}
{{- define "fp-pipeline-orchestrator.chart" -}}
{{- printf "%s-%s" .Chart.Name .Chart.Version | replace "+" "_" | trunc 63 | trimSuffix "-" -}}
{{- end -}}

{{/*
fp-pipeline-orchestrator.selectorLabels — the IMMUTABLE identity labels.
...
*/}}
{{- define "fp-pipeline-orchestrator.selectorLabels" -}}
app.kubernetes.io/name: {{ include "fp-pipeline-orchestrator.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
{{- end -}}

{{/*
fp-pipeline-orchestrator.labels — the FULL recommended label set (app.kubernetes.io/*).
...
*/}}
{{- define "fp-pipeline-orchestrator.labels" -}}
helm.sh/chart: {{ include "fp-pipeline-orchestrator.chart" . }}
{{ include "fp-pipeline-orchestrator.selectorLabels" . }}
{{- if .Chart.AppVersion }}
app.kubernetes.io/version: {{ .Chart.AppVersion | quote }}
{{- end }}
app.kubernetes.io/managed-by: {{ .Release.Service }}
app.kubernetes.io/part-of: forgepoint
app.kubernetes.io/component: pipeline-orchestrator
{{- end -}}

{{/*
fp-pipeline-orchestrator.serviceAccountName — the SA to attach to pods.
...
*/}}
{{- define "fp-pipeline-orchestrator.serviceAccountName" -}}
{{- if .Values.serviceAccount.create -}}
{{- default (include "fp-pipeline-orchestrator.fullname" .) .Values.serviceAccount.name -}}
{{- else -}}
...
{{/*
fp-pipeline-orchestrator.secretName — the Secret the Deployment envFrom's.
...
*/}}
{{- define "fp-pipeline-orchestrator.secretName" -}}
{{- if .Values.secrets.create -}}
{{- include "fp-pipeline-orchestrator.fullname" . -}}
{{- else -}}
...
{{/*
fp-pipeline-orchestrator.image — assemble repository:tag with appVersion as the tag
fallback. Centralized so every manifest (and any future init container) references
the exact same image string.
*/}}
{{- define "fp-pipeline-orchestrator.image" -}}
{{- $tag := default .Chart.AppVersion .Values.image.tag -}}
{{- printf "%s:%s" .Values.image.repository $tag -}}
{{- end -}}

{{/*
fp-pipeline-orchestrator.validateJwtSecret — fail-closed validation for the shared HMAC-SHA256 key.
...
the validated key so call sites do `{{ include "fp-pipeline-orchestrator.validateJwtSecret" . | quote }}`.
*/}}
{{- define "fp-pipeline-orchestrator.validateJwtSecret" -}}
