...
{{/*
fp-auth.name — the chart's short name, overridable, truncated to the 63-char
...
{{/*
fp-auth.fullname — the fully-qualified resource name (release + chart).
WHY this exact dance (the canonical Helm idiom):
  ...
  - else if the release name already contains the chart name, don't double it
    (release "fp-auth" + chart "fp-auth" → "fp-auth", not "fp-auth-fp-auth").
  - else join release + chart.
Always trunc 63 / trimSuffix "-" for DNS-1123 compliance.
*/}}
{{- define "fp-auth.fullname" -}}
...
{{/*
fp-auth.chart — "name-version" sanitized for the helm.sh/chart label
(+ → _ because '+' is illegal in a label value; appears in SemVer build metadata).
*/}}
{{- define "fp-auth.chart" -}}
{{- printf "%s-%s" .Chart.Name .Chart.Version | replace "+" "_" | trunc 63 | trimSuffix "-" -}}
{{- end -}}

{{/*
fp-auth.selectorLabels — the IMMUTABLE identity labels.
...
*/}}
{{- define "fp-auth.selectorLabels" -}}
app.kubernetes.io/name: {{ include "fp-auth.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
{{- end -}}

{{/*
fp-auth.labels — the FULL recommended label set (app.kubernetes.io/*).
...
*/}}
{{- define "fp-auth.labels" -}}
helm.sh/chart: {{ include "fp-auth.chart" . }}
{{ include "fp-auth.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
{{/*
fp-auth.serviceAccountName — the SA to attach to pods.
...
*/}}
{{- define "fp-auth.serviceAccountName" -}}
{{- if .Values.serviceAccount.create -}}
{{- default (include "fp-auth.fullname" .) .Values.serviceAccount.name -}}
{{- else -}}
...
{{/*
fp-auth.secretName — the Secret the Deployment envFrom's.
...
*/}}
{{- define "fp-auth.secretName" -}}
{{- if .Values.secrets.create -}}
{{- include "fp-auth.fullname" . -}}
{{- else -}}
...
{{/*
fp-auth.image — assemble repository:tag with appVersion as the tag fallback.
Centralized so every manifest (and any future init container) references the
exact same image string.
*/}}
{{- define "fp-auth.image" -}}
{{- $tag := default .Chart.AppVersion .Values.image.tag -}}
{{- printf "%s:%s" .Values.image.repository $tag -}}
{{- end -}}

{{/*
fp-auth.validateJwtSecret — fail-closed validation for the shared HMAC-SHA256 key.
...
the validated key so call sites do `{{ include "fp-auth.validateJwtSecret" . | quote }}`.
*/}}
{{- define "fp-auth.validateJwtSecret" -}}
