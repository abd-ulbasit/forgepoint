...
file is the fp-auth reference _helpers verbatim with only the chart-name in the
template identifiers changed (fp-auth.* → fp-model-monitor.*) and the component
label set to "model-monitor".
...
{{/*
fp-model-monitor.name — the chart's short name, overridable, truncated to the
63-char DNS label limit. trimSuffix "-" avoids a trailing dash if truncation
lands on one.
*/}}
{{- define "fp-model-monitor.name" -}}
{{- default .Chart.Name .Values.nameOverride | trunc 63 | trimSuffix "-" -}}
{{- end -}}

{{/*
fp-model-monitor.fullname — the fully-qualified resource name (release + chart).
WHY this exact dance (the canonical Helm idiom):
  ...
  - else if the release name already contains the chart name, don't double it
    (release "fp-model-monitor" + chart "fp-model-monitor" → "fp-model-monitor").
  - else join release + chart.
Always trunc 63 / trimSuffix "-" for DNS-1123 compliance.
*/}}
{{- define "fp-model-monitor.fullname" -}}
...
{{/*
fp-model-monitor.chart — "name-version" sanitized for the helm.sh/chart label
(+ → _ because '+' is illegal in a label value; appears in SemVer build metadata).
*/}}
{{- define "fp-model-monitor.chart" -}}
{{- printf "%s-%s" .Chart.Name .Chart.Version | replace "+" "_" | trunc 63 | trimSuffix "-" -}}
{{- end -}}

{{/*
fp-model-monitor.selectorLabels — the IMMUTABLE identity labels.
...
*/}}
{{- define "fp-model-monitor.selectorLabels" -}}
app.kubernetes.io/name: {{ include "fp-model-monitor.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
{{- end -}}

{{/*
fp-model-monitor.labels — the FULL recommended label set (app.kubernetes.io/*).
...
*/}}
{{- define "fp-model-monitor.labels" -}}
helm.sh/chart: {{ include "fp-model-monitor.chart" . }}
{{ include "fp-model-monitor.selectorLabels" . }}
{{- if .Chart.AppVersion }}
app.kubernetes.io/version: {{ .Chart.AppVersion | quote }}
{{- end }}
app.kubernetes.io/managed-by: {{ .Release.Service }}
app.kubernetes.io/part-of: forgepoint
app.kubernetes.io/component: model-monitor
{{- end -}}

{{/*
fp-model-monitor.serviceAccountName — the SA to attach to pods.
...
*/}}
{{- define "fp-model-monitor.serviceAccountName" -}}
{{- if .Values.serviceAccount.create -}}
{{- default (include "fp-model-monitor.fullname" .) .Values.serviceAccount.name -}}
{{- else -}}
...
{{/*
fp-model-monitor.secretName — the Secret the Deployment envFrom's.
...
*/}}
{{- define "fp-model-monitor.secretName" -}}
{{- if .Values.secrets.create -}}
{{- include "fp-model-monitor.fullname" . -}}
{{- else -}}
...
{{/*
fp-model-monitor.image — assemble repository:tag with appVersion as the tag
fallback. Centralized so every manifest (and any future init container)
references the exact same image string.
*/}}
{{- define "fp-model-monitor.image" -}}
{{- $tag := default .Chart.AppVersion .Values.image.tag -}}
{{- printf "%s:%s" .Values.image.repository $tag -}}
{{- end -}}

{{/*
fp-model-monitor.validateJwtSecret — fail-closed validation for the shared HMAC-SHA256 key.
...
the validated key so call sites do `{{ include "fp-model-monitor.validateJwtSecret" . | quote }}`.
*/}}
{{- define "fp-model-monitor.validateJwtSecret" -}}
