...
// this workspace. Reading the file + pool.Exec needs nothing beyond pgx (already
// required) and os (stdlib) — zero new deps, and it exercises the EXACT SQL that
// ships. The schema under test is therefore the production schema, file-for-file.
//
...
// migrationsPath resolves the service's migrations directory RELATIVE TO THIS SOURCE
// FILE via runtime.Caller, not the test's working directory. `go test` sets CWD to
// the package dir, but `go test ./...` and some IDE runners do not — anchoring to the
// source file makes the path robust regardless of how the suite is invoked. This file
// lives at internal/repository/postgres/migrate_test.go, so migrations are three dirs
// up (../../../migrations).
...
// applyMigrations reads every NNN_*.up.sql file (sorted) and executes it against the
// pool. Called by every integration test after starting the container, so each test
// runs against a freshly-migrated, production-shaped schema.
func applyMigrations(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	...
	sort.Strings(upFiles) // lexical = numeric order given the zero-padded NNN_ prefix
	if len(upFiles) == 0 {
