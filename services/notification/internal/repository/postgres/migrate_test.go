...
// SOURCE FILE via runtime.Caller, not the test's working directory. `go test` sets
// CWD to the package dir, but `go test ./...` and some IDE runners do not —
// anchoring to the source file makes the path robust regardless of how the suite is
// invoked. This file lives at internal/repository/postgres/migrate_test.go, so
...
// the pool. Called by every integration test (via newStore) after starting the
// container, so each test runs against a freshly-migrated, production-shaped schema.
